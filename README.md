...
支持 git commit / PR / release 的可移植 skill，内置全量 gitmoji catalog，默认使用 gitmoji + Conventional Commit，支持中文或英文提交，默认不添加 AI co-author 信息。
...
- 默认输出：gitmoji + Conventional Commit，例如 `✨ feat(scope): summary`
- 明确要求不要 emoji / ASCII-only：回退到 plain Conventional Commit，例如 `feat(scope): summary`
- shell / terminal / 编码环境不确定：回退到 ASCII-only Conventional Commit
- commit title 与 PR title 默认沿用同一种风格，除非用户明确要求不同

## 触发规则

当用户使用这个 skill 进行提交相关工作时，默认采用 gitmoji + Conventional Commit。
只有在用户明确要求以下情况时，才关闭 emoji：
- `no emoji`
- `ASCII-only`
